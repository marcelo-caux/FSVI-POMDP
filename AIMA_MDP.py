...
#        grid.reverse()  # because we want row 0 on bottom, not on top
        ...
        for x in range(self.cols):
            for y in range(self.rows):
                if grid[y][x]:
                    ...
                    states.add((x, y))
                    reward[(x, y)] = grid[y][x]
                    self.terminals_reverse(x,y,grid[y][x])
                 #   states.add((y, x))
                 #   reward[(y, x)] = grid[y][x]
                 #   self.terminals_reverse(y,x,grid[y][x])
                else:
                    self.muro_reverse(x,y,grid[y][x])
                 #   self.muro_reverse(y,x,grid[y][x])
        self.states = states
        terminals=self.termrev
    ...
    # added function to reverse terminals accordingly due to the variable grid
    def terminals_reverse(self, x, y, val):
        if val != 0.01:
            self.termrev.append((x,y))
        return None

    # added function to reverse Barriers (muros) after grid reverse, those coordinates will be used in the visual grid
    def muro_reverse(self,x,y,val):
        if val == None :
            self.murorev.append((x,y))
        return None
